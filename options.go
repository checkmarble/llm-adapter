...
// WithDefaultModel sets the model to use if not specified in
// a particular request. It is the caller's responsibility to ensure
// the requested model is available on the configured provider.
...
// WithApiKey sets an API key to use for every request to the provider.
// Note that not all providers support API key authentication and a given
// provider will only use this if it requires it or ignore it otherwise.
//
// For provider needing specific authentication, an specific option will
// be available on the provider itself.
...
// WithSaveContext enabled history accumulation for any messages sent to
// the provider. If this is not called, every request to the provider will
// have a blank context.
